...
# enumerate函数会对输入的列表进行枚举，生成一个包含元素索引和元素值的枚举对象。每个元素都是一个元组，第一个元素是索引（从0开始），第二个元素是原列表中的单词。
# 字典推导式，它会遍历enumerate生成的枚举对象中的每个元组，将元组中的单词作为键，将索引作为值，创建一个新的字典。

# 列表推导式，遍历创建列表，为zidian_x的key
